...
public class FirebaseReceiver extends FirebaseMessagingService {
    ...
    static final String MessageStatusBroadcast = "com.leagueofshadows.enc.Message_status_broadcast";
    ...
    public void onMessageReceived(@NonNull RemoteMessage remoteMessage) {
        ...
        if(data.containsKey(RECEIVED_STATUS)||data.containsKey(SEEN_STATUS))
        {
            ...
            String userId = data.get(Util.userId);
            DatabaseManager.initializeInstance(new SQLHelper(getApplicationContext()));
            if(data.containsKey(RECEIVED_STATUS))
                DatabaseManager.getInstance().updateMessageReceivedStatus(data.get(RECEIVED_STATUS),id);
            else
                DatabaseManager.getInstance().updateMessageSeenStatus(data.get(SEEN_STATUS),id);
    ...
    public void onNewToken(@NonNull String s) {
        ...
        String userId = sp.getString(RESTHelper.USER_ID,null);
        ...
        if(userId!=null && accessToken!=null)
        {
            ...
            params.put(RESTHelper.USER_ID,userId);
